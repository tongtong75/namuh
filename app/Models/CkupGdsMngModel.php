...
class CkupGdsMngModel extends Model
{
    ...
    protected $allowedFields    = [
        'HSPTL_SN', 'CO_SN', 'CKUP_YYYY', 'CKUP_GDS_NM', 'SPRT_SE', 'FAM_SPRT_SE', 'DEL_YN', 'REG_ID', 'MDFCN_ID'
    ];
    ...
    public function getCkupGdsWithDetail(int $id): ?array
    {
        ...
        foreach ($choiceGroups as &$group) {
            $group['items'] = $this->db->table('CKUP_GDS_CHC_ARTCL a')
                ->join('CKUP_ARTCL_MNG b', 'a.CHC_ARTCL_SN = b.CKUP_ARTCL_SN')
        ...
        // 4. 추가 선택 항목 조회
        $addChoiceItems = $this->db->table('CKUP_GDS_ADD_CHC a')
            ->join('CKUP_ARTCL_MNG b', 'a.CHC_ARTCL_SN = b.CKUP_ARTCL_SN')
            ->where('a.CKUP_GDS_SN', $id)
            ->select('b.*')
            ->get()->getResultArray();
