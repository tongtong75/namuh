...
class CkupTrgtController extends BaseController
{
    ...
    {
        $data['title'] = '검진대상자 관리';
        $data['companies'] = $this->coMngModel->where('DEL_YN', 'N')->orderBy('CO_NM', 'ASC')->findAll();
        $currentYear = date('Y');
        $data['years'] = range($currentYear + 1, $currentYear - 1);
        return view('mngr/ckup_trgt/index', $data);
    }
    ...
    public function ajax_list()
    {
        if ($this->request->isAJAX()) {
            ...
            $extraWhere = [];
            if (!empty($requestData['co_sn_filter'])) {
                $extraWhere['CT.CO_SN'] = $requestData['co_sn_filter'];
            }
    ...
    {
        if ($this->request->isAJAX()) {
            $item = $this->ckupTrgtModel->where('DEL_YN', 'N')->find($id);
            if ($item) {
    ...
    public function excel_upload()
    {
        ...
        $coSn     = $this->request->getPost('fileModal_co_sn');
    ...
    public function excel_download()
    {
        ...
        $coSn = $this->request->getGet('co_sn_filter');
    ...
    public function ajax_reset_password($id = null)
    {
        if ($this->request->isAJAX() && $this->request->getMethod() === 'POST') {
            ...
            $item = $this->ckupTrgtModel->find($id);
            if (!$item) {
                return $this->response->setJSON(['status' => 'error', 'message' => '대상자 정보를 찾을 수 없습니다.', 'csrf_hash' => csrf_hash()]);
            }
